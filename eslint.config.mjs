...
const IS_JS_TYPED = Boolean((JSON.parse(readFileSync(path.resolve(path.join(import.meta.dirname, "tsconfig.json")), "utf-8"))).compilerOptions?.checkJs);

console.log(`Linting JS files with type inference: ${IS_JS_TYPED}`);
...
function getTsTypedOrNonTypedRules(is_typed) {
    const rules = {
        // There is 1 typed-or-non-typed rules under @typescript-eslint
        // setted: 1/1
        // TODO: move rules from before that are in typed rules but that can also works non-typed with their value set to non-typed here and change them later
...
export default defineConfig([
    // Basic config
    {
        ...
        // TODO: add more language and parser options (but set those that require type in TS config, but can let here those that are for type but do not enfore it)
        languageOptions: {
            parser: tsParser,
            parserOptions: {
                projectService: true,
        ...
        // TODO: add more settings according to plugins used and basic from eslint (but set those that require type in TS config, but can let here those that are for type but do not enfore it)
        settings: {},
        rules: {
            ...ESLINT_RULES,
            ...STYLISTIC_RULES,
